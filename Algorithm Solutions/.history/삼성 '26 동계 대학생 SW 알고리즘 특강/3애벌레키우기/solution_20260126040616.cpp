...
void tick() {
    for (int i = 0; i < larvas.size(); ++i) {
        ...
        if (larvas[i].lines.size() == 1) {
            ...
            line.len = 1;
            line.x = larvas[i].lines.back().x + larvas[i].lines.back().len * dx[larvas[i].lines.back().dir] + dx[line.dir];
            line.y = larvas[i].lines.back().y + larvas[i].lines.back().len * dy[larvas[i].lines.back().dir] + dy[line.dir];
            larvas[i].lines.push_back(line);
        } else {
            larvas[i].lines.back().x += dx[larvas[i].lines.back().dir];
            larvas[i].lines.back().y += dy[larvas[i].lines.back().dir];
        }
