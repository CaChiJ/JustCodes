...
bool canScore();
int calcScore(int, int, int);
int calcHorizontalCount(int, int, int*, int*);
int calcVerticalCount(int, int, int*, int*);
void removeAll();
int getScore(int);
void markSequence(bool (*)[WIDTH]);
int scoreFromMarking(bool (*)[WIDTH]);
...
int* takeTurn()
{
	...
	while (true) {
		...
		int score = scoreFromMarking(marking);
...
void markSequence(bool (*marking)[WIDTH]) {
	for (int col = 0; col < WIDTH; ++col) {
		int lastTile = -1;
		int length = 0;
		...
		for (int row = 0; row < HEIGHT; ++row) {
			if (lastTile != tiles[row][col]) {
				...
				if (3 == length) {
					marking[row-2][col] = marking[row-1][col] = marking[row][col] = true;
				} else if (3 < length) {
	...
	for (int row = 0; row < HEIGHT; ++row) {
		...
		for (int col = 0; col < WIDTH; ++col) {
			if (lastTile != tiles[row][col]) {
				...
				if (3 == length) {
					marking[row][col-2] = marking[row][col-1] = marking[row][col] = true;
				} else if (3 < length) {
					marking[row][col] = true;
				}
			}
...
int scoreFromMarking (bool (*marking)[WIDTH]) {
	int score = 0;
	...
	for (int row=0; row < HEIGHT; ++row) {
		...
		for (int col=0; col < WIDTH; ++col) {
			...
			length++;
			score += getScore(length) - getScore(length-1);
		}
	}
